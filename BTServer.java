   ...
   class Gmanager extends Thread {
       ...
       public void msgChecking(String msg){
          ...
          if(code.equals("@" + Setting.CHAT)){
			 ...
			 String firstidx = msg.substring(3);
             if(firstidx.equals("0")){
				 playNumber++;
				 P.pln("한번만...PlayNumber:"+playNumber);
				 if(playNumber > 3) { //3회이상 재생되면, pass
					 broadcast("#"+Setting.COUNT+" 아쉽네요! PASS! \n 다음 음악을 틀게요!");
					 try{
						Thread.sleep(1000);
					 }catch(InterruptedException ie){ }
       ...
       void randomSongNumber(String genre){
          playNumber=0;
          P.pln((playNumber+1)+"회 재생 중.");
          Random r = new Random();
          MusicStart = true;
          gameNumber++;
          if(gameNumber > 4){ //5판 하면 끝남.
             broadcast("#"+Setting.GAME_OVER);
