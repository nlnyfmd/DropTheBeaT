...
public class BTServer extends JFrame implements ActionListener {
	...
	ServerSocket Sers;
	Socket soc;
	int ready, charac, score, MusicEnd, playNumber, gameNumber, randomN, status;
	int port = 2222;
	String ans,genre;
	...
	public class Gmanager extends Thread {
		 ...
		 public void resetList(){
			 ...
			 idx = 0;
			 for(Map.Entry<String,Integer> cEntry : cCharac.entrySet()) charackey[idx] = cEntry.getValue();
			 for(int i =0; i<clist.size(); i++) broadcast("#" + Setting.RESET + keys[i] + "/" +values[i] + "," + charackey[i]+" "+i);
		 }
		 ...
		 public void msgChecking(String msg){
			 ...
			 if(code.equals("@" + Setting.CHAT)){
				 ...
				 readmsg(songMsg);
			 }else if(code.equals("@" + Setting.NEXT) && msg.substring(3).equals("1")){ // 한번 재생
				 broadcast("#" + Setting.MUSIC+"/"+randomN);
				 gameNumber++;
			 }else if(code.equals("@" + Setting.NEXT) && msg.substring(3).equals("2")){

			 }
		 }
		 /*
		 void gameCount(){
			 if(status == Setting.ING && playNumber == 1){
				 status = Setting.AFTER;
				 if(status == Setting.AFTER){
					 status=Setting.BEFORE;
					 P.pln("gameNumber : "+gameNumber);
					 P.pln("게임넘버제발한번만찍히길");
					 broadcast("#" + Setting.MUSIC+"/"+randomN);
				 }
			 }
		 ...
		 void randomAuthor(){
			 if(gameNumber > 5){ //5판 하면 끝남.
				 broadcast("#"+Setting.GAME_OVER);
				 broadcast("#"+Setting.CHAT+" * 게임이 종료되었습니다! ");
				 MusicStart = false;
				 gameNumber = 0;
				 ready = 0;
				 genre = "";
		 ...
		 public void randomSongNumber(String genre){
			 ...
			 if(genre.equals("댄스")) {
				 if(randomN!=0){
					randomN = randomN+dance;
				 }
			 }else if(genre.equals("힙합")) {
				 if(randomN!=0) {
					 randomN = randomN+hiphop;
				 }else{
					 randomN = 5;
				 }
			 }else if(genre.equals("트로트")) {
				 if(randomN!=0) {
					 randomN = randomN+trot;
				 }else{
					 randomN = 10;
				 }
