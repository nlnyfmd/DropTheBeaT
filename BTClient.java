...
public class BTClient extends JFrame implements ActionListener {
   ...
   int port = 2222;
   Socket soc;
   String smusic, name, score, idx,charac,characName,sendmsg,line,genre,songNumber,songTTitle,myIdx,nick;
   ...
   public BTClient(){
    ...
    void enter(){
      nick = Start.Nickname; //여기서 로그인 클래스의 데이터 받아옴. 
      String ip = Start.ip;
   ...
   class Listening extends Thread {
               ...
               if(code.equals("#"+Setting.RESET)){
                  ...
                  try{ // 해당 번호의 노래를 트는 곳
                     ...
                     try{ // 노래 1회차 재생 다 됐다고 알려주기.
                        dos.writeUTF("@"+Setting.NEXT+myIdx);
                        dos.flush();
                     }catch(IOException ii){ }
      ...
      public void renewalList(){
         ...
         if(indexN == 0){
			...
			if(name.equals(Start.Nickname)) listset(charaN);
         }else if(indexN == 1){
